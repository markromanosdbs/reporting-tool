        ...
        <table className="w-full text-sm" style={{ borderCollapse: 'collapse', borderSpacing: 0 }}>
          <thead style={{ borderTop: '2px solid black' }}>
            ...
            {tableName === 'roller_blind_components' && (
              <tr style={{ backgroundColor: '#fff2cc', borderBottom: '1px solid white', height: '60px' }}>
            ...
            {tableName === 'roller_blind_components' && (
              <tr style={{ backgroundColor: '#c0e0f8', borderBottom: '1px solid white', height: '60px' }}>
                {(() => {
            ...
            {(tableName === 'door_screen_components' || tableName === 'roller_shutter_components' || tableName === 'external_blinds_components') && (
              <tr style={{ backgroundColor: '#fff2cc', borderBottom: '2px solid #d4a400', height: '60px' }}>
            ...
            {(tableName === 'door_screen_components' || tableName === 'roller_shutter_components' || tableName === 'external_blinds_components') && (
              <tr style={{ backgroundColor: '#b3e5fc', borderBottom: '2px solid #0277bd', height: '60px' }}>
                {(() => {
                  const headers = table.getHeaderGroups()[0]?.headers || [];
                  const result: JSX.Element[] = [];
                  let i = 0;
                  ...
                  while (i < headers.length) {
                    ...
                    if (isFirstColumn) {
                      ...
                      result.push(
                        <td key={`gh-${currentHeader.id}`} style={{ padding: '4px 2px', backgroundColor: '#f5f5f5', border: '1px solid white', textAlign: 'center', verticalAlign: 'middle', color: '#000000', fontSize: '11px', fontWeight: 'bold', minWidth: '55px', maxWidth: '55px', overflow: 'visible' }}>
                        </td>
                      );
                      i++;
                    } else {
            ...
            {(tableName === 'roller_blind_components' || tableName === 'door_screen_components' || tableName === 'roller_shutter_components' || tableName === 'external_blinds_components') && (
              <tr style={{ backgroundColor: '#d8bfd8', borderBottom: '2px solid #8b7ba8', height: '95px' }}>
                {table.getHeaderGroups()[0]?.headers.map((header, idx) => {
                  const headerText = String(header.column.columnDef.header || '');
                  const isBaseColumn = BASE_COLUMNS.some(
                    (base) => base.toLowerCase() === headerText.toLowerCase()
